...
from Myvelo.basetrainer import BaseTrainer
from Myvelo._utils import inf_loop, MetricTracker
...
class Trainer(BaseTrainer):
    ...
    def _compute_core(self, batch_data):
        ...
        #     "Ux_sz": self.Ux_sz[i],
        #     "Sx_sz": self.Sx_sz[i],
        #     'TFexpr': self.TFexpr[i],
        #     "inputdata": self.inputdata[i],
        #     "mask_u": self.mask_u[i],
        #     "index": i,
        # }
        data_dict = batch_data
        x_u, x_s, y, inputdata, mask_u, TFexpr = data_dict["Ux_sz"], data_dict["Sx_sz"], data_dict['y'], data_dict["inputdata"], data_dict["mask_u"], data_dict['TFexpr']
        y, inputdata, mask_u, TFexpr = (
            y.to(self.device),
            inputdata.to(self.device),
            mask_u.to(self.device),
            TFexpr.to(self.device),
        )

        # In InterVELO():
        # def forward(self, y, input_data, mask_u, TFexpr):
        loss, loss_dict, pseudotime, pred = self.model(y, inputdata, mask_u, TFexpr)
        return loss, loss_dict, pseudotime, pred
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch_data in enumerate(loader):
            loss, loss_dict, pseudotime, mean_pred = self._compute_core(batch_data)
    ...
    def _get_alpha(
        ...
        model.eval()
        alpha_dict = model.decoder(Z,TFexpr, T)
        alpha = alpha_dict['alpha']
        # print(f'alpha: {alpha}, alpha_from_latent: {alpha_dict["alpha_from_latent"]}')
        return alpha, alpha_dict["alpha_from_latent"], alpha_dict['alpha_from_TF']
    ...
    def eval(self, eval_loader, return_kinetic_rates=False):
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(loader):
                TFexpr = batch_data['TFexpr']
                
                loss, loss_dict, pseudotime_, mean_pred= self._compute_core(batch_data)
                ...
                if return_kinetic_rates:
                    cur_alpha_rates, cur_alpha_latent, cur_alpha_TF = self._get_alpha(self.model, pseudotime, TFexpr, pred_z)
                    alpha_rates.append(cur_alpha_rates.cpu().data)
                    alpha_rates_latent.append(cur_alpha_latent.cpu().data)
                    alpha_rates_TF.append(cur_alpha_TF.cpu().data)
            ...
            alpha_rates_latent = np.concatenate(alpha_rates_latent, axis=0)
            alpha_rates_TF = np.concatenate(alpha_rates_TF, axis=0)
        ...
        return velo_mat, velo_mat_u, alpha_rates, kinetic_rates, pseudotime, mix_z, vector_field, alpha_rates_latent, alpha_rates_TF