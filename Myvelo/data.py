...
class VeloDataset(Dataset):
    def __init__(
        ...
        try:
            TF_list = adata.uns['valid_TF_names']
            self.TFexpr = adata[:, TF_list].X.toarray()
            print(f'TFexpr shape: {self.TFexpr.shape}')
        except KeyError:
            print('valid_TF_names not in adata.uns')

        self.Ux_sz = torch.tensor(self.Ux_sz, dtype=torch.float32)
        self.Sx_sz = torch.tensor(self.Sx_sz, dtype=torch.float32)
        self.y = torch.tensor(self.y, dtype=torch.float32)
        self.TFexpr = torch.tensor(self.TFexpr, dtype=torch.float32)
